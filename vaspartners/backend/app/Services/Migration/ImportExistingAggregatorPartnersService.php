 ...
 *
 * Companies / TIN / owner contacts are completed later during TIN verification.
 * Duplicate key: Service ID (zero-pad variants collapsed via finder).
 */
class ImportExistingAggregatorPartnersService
{
     ...
     * }
     */
    public function import(string $path, bool $dryRun = false, bool $linkCompanies = true): array
    {
        ...
        $runner = function () use ($partners, $catalogIds, $dryRun, $linkCompanies, &$stats): void {
    ...
    protected function findCompanyIdForName(string $partnerName): ?int
    {
        ...
        // Prefer exact case-insensitive name, then fuzzy among a narrowed set.
        $exact = Company::query()
