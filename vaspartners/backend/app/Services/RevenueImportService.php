...
class RevenueImportService
{
    ...
    /**
     * Build a monthly revenue import from the AM's Revenue Partners list.
     *
     * @param  list<array{revenue_partner_id: int, amount: float|int|string}>  $lines
     */
    public function createFromPartners(
        User $actor,
        string $period,
        int $vasServiceId,
        ...
        $service = \App\Models\Service::query()->find($vasServiceId);
        if (! $service) {
            throw ValidationException::withMessages(['vas_service_id' => 'Catalog service is required.']);
        }

        $normalizedLines = [];
        foreach ($lines as $index => $line) {
            $partnerId = (int) ($line['revenue_partner_id'] ?? 0);
            $amountRaw = $line['amount'] ?? null;
            if ($partnerId <= 0) {
                throw ValidationException::withMessages([
                    'lines' => 'Each line needs a revenue partner (row '.($index + 1).').',
                ]);
            }
            if (! is_numeric($amountRaw) || (float) $amountRaw <= 0) {
                throw ValidationException::withMessages([
                    'lines' => 'Each partner needs an amount greater than 0 (row '.($index + 1).').',
                ]);
            }
            $normalizedLines[] = [
                'revenue_partner_id' => $partnerId,
                'amount' => round((float) $amountRaw, 4),
            ];
        }

        if ($normalizedLines === []) {
            throw ValidationException::withMessages(['lines' => 'Select at least one partner with an amount.']);
        }

        $partnerIds = collect($normalizedLines)->pluck('revenue_partner_id')->unique()->values()->all();
        if (count($partnerIds) !== count($normalizedLines)) {
            throw ValidationException::withMessages(['lines' => 'Duplicate partners in the list.']);
        }

        $partnersQuery = RevenuePartner::query()
            ->whereIn('id', $partnerIds)
            ->where('is_active', true);

        if (! $actor->canAccessAllRevenue()) {
            $partnersQuery->where('created_by_user_id', $actor->id);
        }

        $partners = $partnersQuery->get()->keyBy('id');
        if ($partners->count() !== count($partnerIds)) {
            throw ValidationException::withMessages([
                'lines' => 'One or more partners were not found in your Revenue Partners list.',
            ]);
        }
        ...
        return DB::transaction(function () use (
            ...
            $import = RevenueImport::query()->create([
                ...
                'period' => $period,
                'vas_service_id' => $vasServiceId,
                'source_filename' => 'from-revenue-partners',
            ...
            foreach ($normalizedLines as $line) {
                ...
                $partner = $partners->get($line['revenue_partner_id']);
                ...
                if (isset($seen[$key])) {
                    RevenueImportRow::query()->create([
                        ...
                        'partner_name' => $partner->partner_name,
                        'amount' => $line['amount'],
                        'amount_raw' => (string) $line['amount'],
                        'status' => RevenueImportRowStatus::Duplicate,
                        'error' => 'Duplicate partner in this import.',
                    ]);
                ...
                if ($this->alreadySentForPartnerPeriod(
                    $partner->id,
                    $serviceId,
                    $period,
                    null,
                ...
                RevenueImportRow::query()->create([
                    ...
                    'status' => $ready
                        ? RevenueImportRowStatus::Matched
                        : RevenueImportRowStatus::MissingPhone,
                    'error' => $ready ? null : 'Partner has no usable phone.',
                ]);
            }

            $import->resolveStatusFromRows();

            return $import->fresh(['vasService', 'creator']) ?? $import;
        });
    }

    /**
     * Correct the billing month (and title / SMS template). Optionally reset sent rows to Ready
     * so SMS can be queued again with the new {period} and wording.
     *
